...
/*   Updated: 2025/02/03 11:18:49 by ggaribot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/03 09:45:20 by ggaribot          #+#    #+#             */
/*   Updated: 2025/02/03 10:53:44 by ggaribot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    // If position is invalid or we hit a wall or already visited cell, return
    if (!is_valid_position(x, y, game) || map[y][x] == '1' || map[y][x] == 'V')
        return;

    // If we can't flood this position, return
    if (!can_flood(map[y][x]))
        return;

    // If we hit an X, mark map as not closed
    if (map[y][x] == 'X')
    {
